...
const { isError, error, data, refetch, isFetching } = useQuery({
  ...
  queryFn: async () => {
    ...
    const { textStream } = streamText({
      ...
      prompt: `I don't have time to read the changelog of the NPM package '${props.name}'. In details tell me about the changes between versions ${props.currentVersion} and ${props.latestVersion}. If there are API breaking changes, use code examples 'before' and 'after'.`,
    });
...
<template>
  <div v-if="!props.openAIAPIKey" class="flex items-center justify-center text-center h-[150px]">
